...
export class AgentHostOctoKitService implements IAgentHostOctoKitService {
	...
	private async _makeGHAPIRequest(
		...
		if (!response.ok) {
			this._logService.error(`[AgentHostOctoKit] ${method} ${url} - Status: ${response.status}`);
			throw new Error(`GitHub API request failed: ${method} ${routeSlug} - ${response.status} ${response.statusText}`);
		}
